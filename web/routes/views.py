...
from strava_integration.map_matcher import match_to_roads
...
def api_normalize(request):
    ...
    if "multipart" in content_type:
        ...
        fit_to_roads = request.POST.get("fit_to_roads", "true").lower() not in ("false", "0", "no")
    else:
        try:
            body = json.loads(request.body)
            route_id = int(body.get("route_id", ""))
        ...
        fit_to_roads = body.get("fit_to_roads", True)
        try:
            gpx_bytes = _fetch_gpx_bytes(route_id)
        except _ApiError as exc:
            return exc.response()
    ...
    if fit_to_roads:
        display_points = match_to_roads(clean_points, radius_m=50.0)
    else:
        display_points = clean_points

    return JsonResponse({
        "original": [[p.lat, p.lon, p.ele] for p in points],
        "normalized": [[p.lat, p.lon, p.ele] for p in display_points],
        "stats": {
            "original_points": len(points),
            "clean_points": len(display_points),
            "detours_removed": len(detours),
            "fit_to_roads": fit_to_roads,
        },
    })
